...
serve(async (req) => {
  ...
  try {
    ...
    if (!["confirmed", "pending"].includes(booking.status)) {
      return new Response(JSON.stringify({ error: "Booking not cancellable" }), {
        status: 409,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (notifyUrl && notifySecret) {
      const { data: bookingDetails } = await admin
        .from("bookings")
        .select("customer_name, customer_email, start_at, services(name), tenants(name, timezone)")
        .eq("id", booking.id)
        .maybeSingle();
        ...
        try {
          await fetch(notifyUrl, {
            ...
            body: JSON.stringify({
              ...
              start_at: bookingDetails?.start_at ?? booking.start_at ?? new Date().toISOString(),
              timezone,
            }),
          });
        } catch {
